...
import { Mail, Lock, User, ArrowRight, Eye, EyeOff, Check, Phone, MapPin } from 'lucide-react';
import './Auth.scss';

export default function Register() {
    const [step, setStep] = React.useState(1); // 1: personal, 2: address, 3: OTP, 4: confirmation
    ...
    const isStep1Valid = data.first_name && data.last_name && data.email && data.gender && data.date_of_birth;
    const isStep2Valid = data.street_address && data.barangay && data.city && data.province && data.zip_code && data.country;
    const isStep3Valid = data.password && data.password === data.password_confirmation && data.phone && data.agree_terms;
            ...
            <div className="auth-card glass-card" style={{ maxWidth: 600 }}>
                <div className="auth-card__header">
                    ...
                    <p className="auth-card__subtitle">
                        Step {step === 4 ? 3 : step} of {step === 4 ? 3 : 3}
                    </p>
                </div>

                <form onSubmit={step === 3 ? handleVerifyOTP : handleSendOTP} className="auth-card__form">
                    {step === 1 && (
                        <>
                            ...
                            <div className="form-group">
                                ...
                                <div className="input-icon-wrapper">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setData('alternative_email', e.target.value)}
                                    />
                                </div>
                            </div>
                    ...
                    {step === 2 && (
                        <>
                            <h3 style={{ marginBottom: 20, fontSize: 18, fontWeight: 600 }}>Address Information</h3>
                    ...
                    {step === 3 && (
                        <>
                            <h3 style={{ marginBottom: 20, fontSize: 18, fontWeight: 600 }}>Password & Agreement</h3>
                            ...
                            <button 
                                type="submit" 
                                disabled={!isStep3Valid || sendingOTP || processing}
                                className="btn-primary auth-card__submit"
                            >
                    ...
                    {step === 3 && otpSent && (
                        <>
                            <div style={{ textAlign: 'center', marginBottom: 24 }}>
                                <div style={{ background: '#E0E7FF', width: 56, height: 56, borderRadius: '50%', margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                    <Check size={28} style={{ color: '#4F46E5' }} />
                                </div>
                            ...
                            <button type="submit" className="btn-primary auth-card__submit" disabled={processing || data.otp.length !== 6}>
                    ...
                    <Link href="/login" style={{ color: '#4F46E5', fontWeight: 600 }}>
                        Sign In
                            ...
                            <button type="submit" className="btn-primary auth-card__submit" disabled={processing}>
                                {processing ? 'Sending OTP...' : 'Send Verification Code'}
                                <ArrowRight size={18} />
                            </button>
                        </>
                    ) : (
